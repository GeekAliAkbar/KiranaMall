...
import { Platform, NavController, ModalController, MenuController, IonRouterOutlet, AlertController } from '@ionic/angular';
...
export class AppComponent {
  ...
  async checkAppVersionUpdates() {
    let redirectURL;
    let data;
    await this.config.getSettingsFromServer().then((response: any) => {
      data = response.data;
    });
    await this.appVersion.getPackageName().then((val) => {
      redirectURL = "https://play.google.com/store/apps/details?id=" + val;
    });
    this.appVersion.getVersionNumber().then((val) => {
      this.appCurrentversion = this.convertVersionToNumber(val);
      if (
        this.appCurrentversion &&
        this.appCurrentversion < this.convertVersionToNumber(data.app_version)
      ) {
        this.showAppVersionMandatoryUpdates(redirectURL);
      }
    });
  }

  showAppVersionMandatoryUpdates(redirectUrl) {
    const buttons = [
      {
        text: 'UPDATE',
        handler: () => {
    ...
    this.presentCustomAlert(
      `Update required`,
      `An update of SurtiBazar is required to continue.`,
      buttons,
    );
  }
  ...
  initializeApp() {
    this.plt.ready().then(() => {
      ...
      this.config.siteSetting().then((value) => {
        ...
        this.shared.dir = localStorage.direction;
        this.checkAppVersionUpdates();
        firebase.initializeApp(this.config.firebaseConfig);
        if (this.shared.customerData && this.shared.customerData.customers_id) {
          this.loadHomePage();
        } else {
          this.loadHomePage();
        }
